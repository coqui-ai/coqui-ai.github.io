...
function deploy_docker_image {
  ...
  update_args+=(--git-url "$INFRASTRUCTURE_REPO_SSH")
...
function run {
  ...
  #Should be run after building
  # for devops local test run (container must exist):
  # aws-vault exec shared --  _ci/scripts/deploy-docker-image.sh us-east-1 test
  # $APP_DEPLOY_PATH $IMAGE_TAG_VARNAME are set in constants.sh
  deploy_docker_image "$region" "$docker_tag" $APP_DEPLOY_PATH $IMAGE_TAG_VARNAME
}
