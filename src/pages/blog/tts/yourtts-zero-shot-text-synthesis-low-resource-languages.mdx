...
name: Edresson Casanova, Eren Gölge, and Julian Weber
...
👉 [Try the YourTTS demo](/)

👉 Visit the YourTTS [project page](https://edresson.github.io/YourTTS/)
...
The recent surge of new end-to-end deep learning models has enabled new and
...
[YourTTS](https://arxiv.org/abs/2112.02418), which can synthesize voices in
...
speakers by zero-shot learning on the fly.

In “**YourTTS**: Towards Zero-Shot Multi-Speaker TTS and Zero-Shot Voice
Conversion for everyone” we introduce a model with the following capabilities:
...
- **Zero-Shot learning**: Adapting the model to synthesize the speech of a novel
  speaker without re-training the model.
- **Speaker/language adaptation**: Fine-tuning a pre-trained model to learn a
  ...
  by transferring knowledge from learned languages)
...
![IMAGE](../../../data/images/blog/posts/blog-tts-yourtts-zero-shot-text-synthesis-low-resource-languages-modes.png)

</div>

### Model Architecture

YourTTS is an extension of our previous work [SC-GlowTTS](https://arxiv.org/abs/2104.05557).
It uses the [VITS](https://arxiv.org/abs/2106.06103) (Variational Inference with
...
of the model. We use the [H/ASP](https://arxiv.org/abs/2009.14153) model as the
speaker encoder architecture. See the figure below for the overall model
architecture in training (right) and inference (left).

<div align="center">

![IMAGE](../../../data/images/blog/posts/blog-tts-yourtts-zero-shot-text-synthesis-low-resource-languages-inference.png)

</div>

VITS is a peculiar TTS model as it employs different deep-learning techniques
...
built on the [GlowTTS](https://arxiv.org/abs/2005.11129) model. The GlowTTS
...
expressivity of the output.

VITS improves on it by introducing specific updates. First, it replaces the
...
We combined multiple datasets for different languages. We used [VCTK](https://datashare.ed.ac.uk/handle/10283/2651)
and [LibriTTS](https://arxiv.org/abs/1904.02882) for English (multispeaker datasets),
[TTS-Portuguese Corpus](https://arxiv.org/abs/2005.05144) (TPC) for Brazilian
Portuguese, and the French subset of the [M-AILABS](https://www.caito.de/2019/01/the-m-ailabs-speech-dataset/)
dataset (FMAI).

We resample the audio clips to 16 kHz, apply voice activity detection to remove
silences and apply [RMS volume normalization](https://github.com/coqui-ai/TTS/blob/aa2450e8f28b4367791a7988cf87e395e501ce67/TTS/utils/audio.py#L787)
before passing them to the speaker encoder.
...
datasets one by one and train for \~120K steps with each new dataset.

Before we report results on each dataset, we also fine-tune the final model with
...
encoder with cosine similarity loss.

We used a single V100 GPU and used a batch size of 64. We used the AdamW
...
performance. We also use [speaker encoder cosine similarity (SECS)](https://arxiv.org/abs/2104.05557) to
...
Please refer to the paper to see the details.

<div align="center">

![IMAGE](../../../data/images/blog/posts/blog-tts-yourtts-zero-shot-text-synthesis-low-resource-languages-table-1.png)

</div>

Table (1) above shows our results on different datasets. Exp1 is trained with
...
with [AttentronZS](https://arxiv.org/abs/2005.08484) and [SC-GlowTTS](https://arxiv.org/abs/2104.05557).
Note that SC-GlowTTS is our previous work which leads the way to YourTTS. You
can find its implementation under [🐸TTS](https://github.com/coqui-ai/TTS). We
...
cases, which is surprising even for us to see.

<div align="center">

![IMAGE](../../../data/images/blog/posts/blog-tts-yourtts-zero-shot-text-synthesis-low-resource-languages-table-2.png)

</div>

Table (2) depicts the zero-shot voice conversion (ZSVC) results between
...
And “M-F” offers the conversion of a Male speaker to a Female speaker.

<div align="center">

![IMAGE](../../../data/images/blog/posts/blog-tts-yourtts-zero-shot-text-synthesis-low-resource-languages-table-3.png)

</div>

Table (3) yields the results for the speaker adaptation experiments where we
...
mere use of speaker embeddings is not enough to produce high-quality results.

Due to the time and space constraints in the paper, we could not expand the
...
Give YourTTS a try right on your browser using the demo on our [homepage](/).

YourTTS is also available in [🐸TTS](https://github.com/coqui-ai/TTS) with a
training recipe and a pre-trained model. You can train your own model,
synthesize voice with the pre-trained model or finetune it with your dataset.
...
misuse of the TTS technology.

To exemplify this, on our demo, we add background music to avert the unintended
use of the voice clips on different platforms.

If you also want to contribute to our research and discussion in this field, join
us [here](https://github.com/coqui-ai/TTS/discussions/1036).
...
We are excited to present YourTTS and see all the different use-cases the 🐸
